...
class Inventory:
    ...
    def merge_query(self, product_codes):
        ...
        # Add this block to fetch the model names for the product codes
        product_code_to_model = item_info.set_index('Item_Code')['Model'].to_dict()
        result.columns = [product_code_to_model.get(col, col) for col in result.columns]
...
print(result)
result.to_excel("D:\TaiCloud\Documents\Documents\Project\SongwooGui\songwoo.xlsx", index=False)
# 엑셀로저장 할수 있는 함수를 만들어주세요.
# result.to_excel("D:/OneDrive/SongwooDB/songwoo.xlsx", index=False)
# 완제품 규격으로 전환완료,
# 엑셀로 저장 완료.